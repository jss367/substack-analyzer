...
def fit_piecewise_logistic(
    ...
    for exog_lag, exog in exog_candidates:
        ...
        for k_combo in product(k_candidates, repeat=num_segments):
            ...
            # Regularize extreme segment slopes so level adjustments are preferred over runaway growth/decay
            r_arr = np.asarray(r_segments, dtype=float)
            scale = tss if tss > 0 else float(np.square(y_vec).sum())

            # Light symmetric L2 penalty on all slopes keeps reasonable growth rates cheap
            slope_l2_strength = 0.02
            slope_penalty = slope_l2_strength * scale * float(np.square(r_arr).sum())

            # Additional hinge penalty for magnitudes above the threshold heavily discourages explosive slopes
            hinge_strength = 0.25
            hinge_threshold = 0.5
            extreme_components = np.clip(np.abs(r_arr) - hinge_threshold, 0.0, None)
            extreme_penalty = hinge_strength * scale * float(np.square(extreme_components).sum())
